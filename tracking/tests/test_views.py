...
def test_workout_session_creation(api_client, create_user):
    user = create_user(username='testuser', password='testpass123')
    plan = WorkoutPlan.objects.create(plan_id='plan1', user=user, plan_name='Plan A', start_date='2023-01-01', end_date='2023-01-30', goals='Lose weight')
    activity = Activity.objects.create(activity_id='activity1', user=user, activity_type='running', duration=30, distance=5.0, date='2023-01-01', calories_burned=300)
    payload = {
        'session_id': 'session1',
